...
namespace VkCore
{
    ...
    ComputePipelineBuilder& ComputePipelineBuilder::BindShaderModule(const ShaderData& shaderData)
    {
        vk::ShaderModuleCreateInfo moduleCreateInfo{};
        moduleCreateInfo.setCodeSize(shaderData.m_Data.size() * sizeof(uint32_t));

        ASSERT(shaderData.m_Data.data() != nullptr || shaderData.m_Data.size() > 0,
               "Couldn't construct a shader module! A pointer to the shader data is null!")

        moduleCreateInfo.setPCode(shaderData.m_Data.data());

        vk::ShaderModule module;

        TRY_CATCH_BEGIN()

        module = VkCore::DeviceManager::GetDevice().CreateShaderModule(moduleCreateInfo);

        TRY_CATCH_END()

        ASSERT(shaderData.m_StageFlags == vk::ShaderStageFlagBits::eCompute, "The shader is not a compute shader!");

        m_ShaderStageCreateInfo.setPName("main");
    ...
    ComputePipelineBuilder& ComputePipelineBuilder::AddDescriptorLayout(const vk::DescriptorSetLayout& layout)
    {

        ASSERT(layout != nullptr, "The given DescriptorLayout is nullptr")
